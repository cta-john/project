...
def test_kb_automation():
    ...
    try:
        ...
        logger.info(f"HTS 경로: {kb_config['실행경로']}") # 진행상황
        logger.info(f"이미지 폴더: {kb_config['이미지폴더']}") # 진행상황

        # 저장 폴더 생성
        save_dir = r"C:\Users\kiwoom\Documents\종투사증거금취합"
        os.makedirs(save_dir, exist_ok=True)
        logger.info(f"저장 폴더: {save_dir}") # 진행상황

        # 1단계: HTS 실행
        print("\n[1단계] KB HTS 실행 중...") # 진행상황
        logger.info("1단계: KB HTS 실행") # 진행상황
        minimize_all_windows()

        proc = Popen([kb_config['실행경로']], stdin=PIPE)
        logger.info(f"HTS 실행 완료 (PID: {proc.pid})")
        print(f"✅ HTS 실행 완료 (PID: {proc.pid})") # 진행상황
        time.sleep(5)

        # 2단계: 로그인 화면 대기
        print("\n[2단계] 로그인 화면 대기 중...") # 진행상황
        logger.info("2단계: 로그인 화면 대기...") # 진행상황
        time.sleep(3)
        print("✅ 로그인 화면 대기 완료") # 진행상황

        # 3단계: ID 입력
        print("\n[3단계] ID 입력 중...") # 진행상황
        logger.info("3단계: ID 입력") # 진행상황
       ...
       # 8단계: 화면번호 0942 입력
        ...
        # confidence=0.8로 설정하여 아주 정확할 때만 위치를 가져옵니다.
        ...
        # 9단계: 다음 버튼 클릭
        print("\n[9단계] 다음 버튼 클릭 중...")
        logger.info("9단계: 다음 버튼 클릭")
        next_button = os.path.join(kb_config['이미지폴더'], "next_button.png")
        field_location = pyautogui.locateOnScreen(next_button, confidence=0.7)
        ...
        if field_location:
            ...
            # 가로 4등분 중 1번째 중앙 = 가로의 1/8 지점
            # 세로 2등분 중 2번째 중앙 = 세로의 3/4 지점
            click_x = left + (width * 0.8)
            click_y = top + (height * 0.5)

            # 계산된 지점 클릭
            print(f"   → 좌표 ({int(click_x)}, {int(click_y)})를 10번 연속 클릭합니다.")
            for i in range(80):
                pyautogui.click(click_x, click_y)
                time.sleep(0.1)  # 클릭 간 간격 (너무 빠르면 인식이 안 될 수 있음)

            logger.info(f"✅ 버튼 10번 연속 클릭 성공: ({click_x}, {click_y})")
            print(f"✅ 버튼 10번 연속 클릭 완료")
        ...
        table_area = os.path.join(kb_config['이미지폴더'], "table_area_0942.png")
        ...
        # 파일명 생성
        today = datetime.now()
        filename = f"{today.year}-{today.month:02d}-{today.day:02d}_KB_0942.csv"
        ...
        # 저장 버튼 클릭
        print("   → 1차 저장 시도 (Enter)")
        pyautogui.press('enter')
        
        # 덮어쓰기 확인 팝업이 올라오는 시간 충분히 대기
        time.sleep(2.0) 

        # 팝업창이 떴을 경우를 대비해 엔터 및 Alt+Y 연속 시도
        print("   → 덮어쓰기 팝업 확인 및 '예(Y)' 승인 중...")
        pyautogui.hotkey('alt', 'y') # 윈도우 표준 '예(Y)' 단축키 강제 입력
        time.sleep(0.5)
        
        logger.info("파일 저장 명령 및 덮어쓰기 처리 완료")
        time.sleep(3.0) # 파일이 실제로 기록될 때까지 여유 있게 대기

        for i in range(10):
            pyautogui.press('esc')
            logger.info(f"ESC 입력 {i+1}회")
            time.sleep(0.5) # 팝업이 닫히는 시간을 고려한 간격
